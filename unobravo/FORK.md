...
| `excalidraw-app/index.scss` | — | one import: `unobravo/theme/accent-orange.scss` repaints the accent palette orange, by redeclaring twelve `theme.scss` custom properties per theme. Nothing in `packages/` is touched. The two blocks are `.excalidraw.excalidraw:not(.theme--dark)` and `.excalidraw.excalidraw.theme--dark`, both 0-3-0 and mutually exclusive, so they outrank every upstream declaration on specificity alone and the bundler's emit order cannot change the outcome — without the `:not()` the light block would tie with upstream's `.excalidraw.theme--dark` at 0-2-0 and the winner would depend on Vite's chunk order. `unobravo/tests/accentPalette.test.ts` is the guard, and it derives the property list from `theme.scss` rather than hardcoding it, so a _new_ upstream accent sibling fails by construction instead of shipping violet — that family has grown twice before. It also fails on a rename, on `!important` appearing upstream, on the light block losing its `:not()`, on `excalidraw-app/index.scss` losing the import, on any violet reappearing in the override, and on `--theme-filter` changing away from `invert(93%) hue-rotate(180deg)` — the dark `--color-selection` is a pre-image of that exact filter, and dropping the hue rotation alone would render the selection azure. Each of those mutations was checked to fail. **Still violet, all of it deliberate:** `COLOR_PALETTE` in `packages/common/src/colors.ts` (drawing swatches — content, not chrome); the violet-tinted greys of the surface scale — `--color-surface-high` and `--color-surface-low`, which alias `--button-hover-bg`, `--button-active-bg`, `--default-border-color`, `--sidebar-border-color` and `filledButtonOnCanvas`, so in light mode a lavender hover sits next to the warm active tool, plus `--color-surface-mid`, the `--button-bg` the slider-track figure is measured against ([#26](https://github.com/unobravo/whiteboard-fe/issues/26)); the canvas literals — seven in `packages/excalidraw/renderer/interactiveScene.ts` (`:166,223,227,229` for line and arrow point handles, `:1244,1265,1269` for arrow-binding focus points), two in `packages/excalidraw/lasso/index.ts:66-67` where the lasso trail is `#6965db` at full opacity, and the `rgba(0, 0, 200, 0.04)` marquee fill at `packages/element/src/renderElement.ts:699,746` — all of which paint violet or blue inside an orange selection ([#27](https://github.com/unobravo/whiteboard-fe/issues/27)); the brand icon set and PWA screenshots ([#28](https://github.com/unobravo/whiteboard-fe/issues/28)); `--color-logo-text`, unreachable while `FEATURES.welcomeLogo` and `FEATURES.plus` are both off (its sibling `--color-logo-icon` aliases `--color-primary` and does turn orange); the two `--color-primary-contrast-offset` values three lines below the import in this very file, left alone because nothing in the tree reads them — the mirror image of [#29](https://github.com/unobravo/whiteboard-fe/issues/29); and the `#6965db` fallback at `packages/excalidraw/components/canvases/InteractiveCanvas.tsx:144`, reached only when the container ref is missing. Hosted here rather than in `excalidraw-app/App.tsx`, which already does `import "./index.scss"` and would have added no register row at all: `App.tsx` took 11 upstream commits last year against this file's 3, and its import block already carries level-1/2/4 fork changes | no |
