...
| --- | --- | --- | --- |
| `.env.production` | — | Sentry disabled, notes on endpoints still pointing at Excalidraw; **SPIKE (this branch only):** drops `VITE_APP_FIREBASE_CONFIG` | no |
| `.env.development` | — | points the collaboration socket at the Unobravo relay; **SPIKE (this branch only):** drops `VITE_APP_FIREBASE_CONFIG` | no |
| `excalidraw-app/collab/Collab.tsx` | 4 | two lines: the relay rejects a handshake without a Firebase ID token, and the socket URL is read from `unobravo/collab/relayUrl.ts` instead of `import.meta.env` directly. **DEMO(MIL-2679) on this branch only:** `FileManager`'s `getFiles`/`saveFiles` no longer reach Firebase Storage — the bytes arrive inside the scene broadcast instead, and `addInlinedFiles` applies them on `SCENE_INIT`/`SCENE_UPDATE`. **SPIKE (this branch only):** drops the `loadFromFirebase` call, the autosave throttle and the `beforeUnload` save-gate — no scene persistence exists anymore, on top of the image path already changed by the DEMO | no |
...
| `excalidraw-app/package.json` | — | drops the `VITE_APP_ENABLE_TRACKING=true` that overrode `.env.production`; **SPIKE (this branch only):** drops the `firebase` dependency | no |
...
| `excalidraw-app/App.tsx` | 1, 2, 4 | imports the overlays, passes the four props, passes `imageOptions={UNOBRAVO_IMAGE_OPTIONS}` (level 1 — see `unobravo/config/imageOptions.ts`), gates Excalidraw+/social/share-link and both the live-collaboration UI and the `<Collab>` mount itself (an iframe embed carrying a relay `authToken` still gets it — see `unobravo/collab/relayAuth.ts` — a bare unauthenticated iframe embed does not, so `#room=` auto-join only works for the Unobravo webapp's own embed, not an arbitrary third-party one); in `initializeScene`, `?id=`/`#json=` are hardcoded to never resolve, so an inbound link can no longer inject scene data via the share backend (MIL-2563) — `#room=` is untouched, it never went through this code path | no |
| `excalidraw-app/collab/Portal.tsx` | 4 | **DEMO(MIL-2679) on this branch only:** `broadcastScene` attaches the dataURLs of the image elements it is syncing, so a peer needs no file store to render them. Only files referenced by the elements in that broadcast go out, so a delta stays a delta. `_broadcastSocketData` also records the payload size in dev (`unobravo/dev/wireStats.ts`) — one seam, the last point at which the payload is plaintext | no |
...
| `excalidraw-app/app_constants.ts` | — | **SPIKE (this branch only):** drops `FIREBASE_STORAGE_PREFIXES`, unused now that scene/file uploads no longer go through Firebase Storage | no |
| `excalidraw-app/vite-env.d.ts` | — | **SPIKE (this branch only):** drops the `VITE_APP_FIREBASE_CONFIG` env var type, paired with the `packages/excalidraw/vite-env.d.ts` row below | no |
| `packages/excalidraw/vite-env.d.ts` | — | **SPIKE (this branch only):** same `VITE_APP_FIREBASE_CONFIG` drop as `excalidraw-app/vite-env.d.ts`; kept in sync per the `excalidraw-app/tsconfig.json` row above | no |
| `excalidraw-app/components/ExportToExcalidrawPlus.tsx` | — | deleted: **SPIKE (this branch only):** uploaded scenes/files to Firebase Storage for the Excalidraw+ import flow; already unreachable with `plus`/`socials` off, so deleted rather than gated | no |
| `excalidraw-app/data/firebase.ts` | — | deleted: **SPIKE (this branch only):** all Firestore/Storage reads and writes this app used, on top of PR #33 | no |
| `excalidraw-app/tests/collab.test.tsx` | — | **SPIKE (this branch only):** drops the mock of `../../excalidraw-app/data/firebase.ts` now that the module is deleted | no |
...
| `packages/excalidraw/types.ts` | 3 | declares `mermaidEnabled`, `libraryEnabled` and `externalLinksEnabled`; **DEMO(MIL-2679):** adds `outputType` to the public `ImageOptions`, and keeps `AppProps["imageOptions"]` from making it required — `undefined` is the meaningful value, it means "keep the source format" | yes |
| `packages/excalidraw/index.tsx` | 3 | defaults and forwards all three props; **DEMO(MIL-2679):** normalizes `imageOptions.outputType` alongside the two fields already there | yes |
| `packages/excalidraw/components/App.tsx` | 3 | clamps a restored `openSidebar` pointing at the removed library tab; **DEMO(MIL-2679):** `initializeImage` passes `outputType` to `resizeImageFile`, and reads `mimeType` _after_ the resize rather than before — with re-encoding on, the old ordering stored `image/png` next to JPEG bytes | yes |
...
- **Collaboration.** Always on, by product decision. A session opens a socket to the relay resolved by `unobravo/collab/relayUrl.ts` (in dev, `VITE_APP_WS_SERVER_URL` — see below) and, on `master`, writes the scene and images to Firebase, which still points at Excalidraw. Repointing Firebase and finding a production relay are go-live prerequisites; `.env.production` says so at each endpoint. **SPIKE (this branch only):** no Firebase write exists to repoint — see the bullet below.
...
- **Firebase is still Excalidraw's.** On `master` this still applies: `saveCollabRoomToFirebase` and the image upload still write to `excalidraw-room-persistence`, and a `socketInitialized` race (Firebase-load vs snapshot-replay) is latent there. **SPIKE (this branch only):** both are gone — `excalidraw-app/data/firebase.ts` is deleted and there is no scene/file store at all, durable or otherwise; the relay's per-room snapshot is now the only source of a joiner's initial state.
