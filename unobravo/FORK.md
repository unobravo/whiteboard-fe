...
Four workflows, all ours: `unobravo-deploy.yml` (every push to `master` builds once and promotes the same bytes staging→production, no approval gate, no `workflow_dispatch`, refuses a run whose commit is no longer `master`'s head); `unobravo-deploy-manual.yml` (`workflow_dispatch` rollback path, not sticky — next push to `master` rolls over it, so revert on `master` to make it permanent); `unobravo-build-app.yml` and `unobravo-deploy-app.yml` (the reusable build and publish).

Deliberate choices: a release is a **commit, not a tag** (`build/version.json`, since tagging would collide with upstream's `v*`); cache headers **follow what Vite hashes** (`fonts/` 30 days, `assets/` immutable 1 year `--size-only`, hashed-name files `no-cache`, nothing ever deleted); the **unit suite gates the deploy** via a racing `test.yml` (`fork:check` is PR-only and pointless once a push to `master` exists); **staging is a gate, not a preview** — publish ends by fetching `version.json` from the host, so `deploy-production: needs: deploy-staging` means staging really serves the build.
