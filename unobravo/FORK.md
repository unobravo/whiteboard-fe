...
| --- | --- | --- | --- |
| `.env.production` | — | Sentry pointed at the Unobravo project (`VITE_APP_DISABLE_SENTRY=false`; the DSN itself arrives as a build-time variable), notes on endpoints still pointing at Excalidraw | no |
...
| `excalidraw-app/sentry.ts` | 4 | reads the DSN from `VITE_SENTRY_DSN` instead of Excalidraw's hardcoded one, resolves the environment through `unobravo/observability/sentryEnv.ts` instead of the `excalidraw.com` / `*.vercel.app` hostname map, and exports whether errors are actually transmitted. `beforeSend` hands the event to `unobravo/observability/scrubSentryEvent.ts`, which upstream had no equivalent of: it scrubbed the fragment off `event.request.url` and nothing else, while `#room=<id>,<key>` carries the key that decrypts the scene and `?authToken=` the relay credential. Passing `integrations` to `Sentry.init` _adds to_ the SDK's defaults rather than replacing them, so the breadcrumb integration records every `history.pushState` with the whole URL and `captureConsoleIntegration` reports whatever a `console.error` was handed — the room key travelled in fields the hook never looked at. The import is the module path, not the `unobravo` barrel: this file is the first import in `excalidraw-app/index.tsx`, and the barrel would pull the feature flags and both collaboration modules into that moment for one function. The environment has to be decided at runtime, not at build time: the pipeline builds once and promotes the identical bytes to both buckets, so a baked-in value would label production errors as staging. Unrecognised hosts — `localhost`, `vite preview`, the CloudFront domain — resolve to no environment and therefore no DSN, which is what keeps a developer's stack traces out of the project | no |
...
| `excalidraw-app/vite-env.d.ts` | — | declares `VITE_SENTRY_DSN`. Only in the app's copy, not `packages/excalidraw/vite-env.d.ts`: both declare the same `ImportMetaEnv`, so one declaration types the app | no |
| `.github/workflows/sentry-production.yml` | — | deleted: upstream's Sentry release job triggers on a `release` branch this fork does not have and uploads `./build/static/js/`, a CRA path that has not existed since Vite. Leaving it in place once the repository holds real `SENTRY_*` credentials meant two Sentry pipelines, one of them broken. The working one is the sourcemap step in `unobravo-build-app.yml` | no |
...
Being deployed turns [what is _not_ gated](#what-is-deliberately-not-gated) from a note into an exposure: collaboration still hits `oss-collab.excalidraw.com` and Excalidraw's Firebase, and `whiteboard.unobravo.com` is public. Inbound `#json=`/`?id=` no longer fetches their share backend (MIL-2563). **Error reporting** now goes to the Unobravo Sentry project (`unobravo-eu`, EU region, issue #16): the DSN comes from the `VITE_SENTRY_DSN` repository variable — public by construction, since it ships inside the bundle — and `unobravo-build-app.yml` fails the build when it is unset, so a deploy cannot quietly lose it. Sourcemaps are uploaded from the same job with `sentry-cli sourcemaps inject && upload`, before the artifact is uploaded so the deployed `.js` carries the debug ids Sentry was told about; the `.map` files themselves still never reach S3 (#12). That upload soft-fails with a warning when `SENTRY_AUTH_TOKEN` is missing — symbolication is an upgrade to error reporting, not a precondition for deploying. `public/robots.txt` now blocks all crawlers; upstream ended it `Allow: /` before `Disallow: /`, and equal-length rules favour `Allow`.
