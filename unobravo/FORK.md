...
| `excalidraw-app/sentry.ts` | 4 | reads the DSN from `VITE_SENTRY_DSN` instead of Excalidraw's hardcoded one, resolves the environment through `unobravo/observability/sentryEnv.ts` instead of the `excalidraw.com` / `*.vercel.app` hostname map, and exports whether errors are actually transmitted. `beforeSend` also scrubs the query string and the fragment from `event.request.url` **and from navigation breadcrumbs**, which upstream left alone: `#room=<id>,<key>` carries the key that decrypts the scene and `?authToken=` the relay credential, and passing `integrations` to `Sentry.init` _adds to_ the SDK's defaults rather than replacing them, so the default breadcrumb integration records every `history.pushState` with the whole URL. Scrubbing `request.url` alone would have sent the room key anyway, in the breadcrumb trail of the same event. The import is the module path, not the `unobravo` barrel: this file is the first import in `excalidraw-app/index.tsx`, and the barrel would pull the feature flags and both collaboration modules into that moment for one function. The environment has to be decided at runtime, not at build time: the pipeline builds once and promotes the identical bytes to both buckets, so a baked-in value would label production errors as staging. Unrecognised hosts — `localhost`, `vite preview`, the CloudFront domain — resolve to no environment and therefore no DSN, which is what keeps a developer's stack traces out of the project | no |
